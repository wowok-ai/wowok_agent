...
export class Account {
    constructor(storage = 'File') {
        ...
        this.balance = async (name, token_type) => {
        ...
        // token_type is 0x2::sui::SUI, if not specified.
        this.coin = async (name, token_type) => {
            const addr = await this.get_address(name);
            if (addr) {
                return (await Protocol.Client().getCoins({ owner: addr, coinType: token_type })).data;
            }
        };
        this.get_coin_object = async (txb, balance_required, name, token_type) => {
            const addr = await this.get_address(name);
        ...
        this.coin_with_balance = async (balance_required, account, token_type) => {
