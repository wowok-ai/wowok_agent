...
export class CallArbitration extends CallBase {
    ...
    async operate(txb, passport, account) {
        ...
        if (this.object_address) {
            ...
            if (!treasury_address) {
                withdraw_treasury = Treasury.New(txb, this.type_parameter, permission ? permission.get_object() : this.permission_address, GetObjectParam(this.data.fee_treasury)?.description ?? '', permission ? undefined : passport);
            }
            ...
            obj = Arbitration.New(txb, this.type_parameter, permission ? permission.get_object() : this.permission_address, this.data?.description ?? '', BigInt(this.data?.fee ?? 0), t, permission ? undefined : passport);
        }
        if (obj) {
